...
class PHEmbedFriendlyThemeHooks implements Gdn_IPlugin {
   ...
   public function OnDisable() {
	...
	public function SettingsController_AfterCurrentTheme_Handler($Sender) {
		$SingleColumn = C('Themes.PHEmbedFriendly.SingleColumn');
		echo Wrap(
			...
			.Wrap(Anchor(
				...
				'settings/phembedfriendlytogglepanel/'.Gdn::Session()->TransientKey(),
				'SmallButton'
			), 'div')
		, 'div', array('class' => 'Description'));
	}
	
	public function SettingsController_PHEmbedFriendlyTogglePanel_Create($Sender) {
	...
	public function PluginController_BeforeEmbedRecommend_Handler($Sender) {
		...
		echo '<div class="EmbedRecommend">
			...
			.Wrap(
				...
				.Wrap(Anchor(
					T($SingleColumn ? 'Show the side panel' : 'Hide the side panel'),
					'plugin/phembedfriendlytogglepanel/'.Gdn::Session()->TransientKey(),
					'SmallButton'
	...
	public function PluginController_PHEmbedFriendlyTogglePanel_Create($Sender) {
		$this->_TogglePanel($Sender);
	...
	private function _TogglePanel($Sender) {
		...
		if (Gdn::Session()->ValidateTransientKey($TransientKey))
			SaveToConfig('Themes.PHEmbedFriendly.SingleColumn', C('Themes.PHEmbedFriendly.SingleColumn') ? FALSE : TRUE);
	}


	public function Base_Render_Before($Sender) {
		if (($Sender->MasterView == 'default' || $Sender->MasterView == '') && C('Themes.PHEmbedFriendly.SingleColumn'))
...
$Sender->Menu->AddLink('Home', Img('themes/PurpleHazeEmbedFriendly/design/images/hicon.png', array('title' => T('Home'))), 'http://www.yoursite.com',  array('class' => 'Button'));
$Sender->Menu->AddLink('Games', Img('/themes/PurpleHazeEmbedFriendly/design/images/smile.png', array('title' => T('Games'))), 'plugin/Games',  array('class' => 'Button'));
...
$Sender->Menu->AddLink('User',  $photo,  '/profile/{UserID}/{Username}',array('Garden.SignIn.Allow'), array('class' => 'UserNotifications'));	
		...
		if (C('Themes.PHEmbedFriendly.SingleColumn'))
